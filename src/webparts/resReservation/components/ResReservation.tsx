...
import { IResReservationState } from "./IResReservationState";
...
const newResEmail = async (to:Array<string>,cc: Array<string>, values: any, type: any,facilitiesAvailable: any,siteUrl: string, id) => {
  let toEmail = [...to];
  let ccEmail = [...cc];
...
export default class ResReservation extends React.Component<
  IResReservationProps,
  IResReservationState
> {
  public inputRef: any;
  public venue: any;
  public layout: any;
  ...
  public handleSave = async (formik) => {
    ...
    if(!Object.keys(formik.errors).length) {
      ...
      await this.CreateRequest(finalResult);
  ...
  public CreateRequest = async (Formdata) => {
    ...
    if(isAvailable) {
      ...
      try {  
        ...
        FromDate: this.ISODate(Formdata["fromDate"]),
        ToDate: this.ISODate(Formdata["toDate"]),
      ...
      await sp.web.lists.getByTitle("ReservationDocs").rootFolder.folders.add(iar.data.GUID)
      .then(r => {
     ...
     if(Formdata["isCSDR"]){
      ...
      if(this.state.isddMember) {
       ...
       await newResEmail([...toUser],[this.state.requestorEmail],
        Formdata, 1, this.state.facilitiesAvailable, this.props.siteUrl, _itemId);
     
     if(this.state.facilityData.length){
       const ownerSet =  {};
       const facilityRecord = this.state.facilityData;
       facilityRecord.forEach((item:any) => {
             if(!ownerSet[this.facilityMap[item.facility].FacilityOwner]){
              ownerSet[this.facilityMap[item.facility].FacilityOwner] = [];
             }
             ownerSet[this.facilityMap[item.facility].FacilityOwner].push(this.facilityMap[item.facility]);
       });
      Object.keys(ownerSet).forEach(async (item) => {
        await newResEmail([item],[], Formdata, 2, 
           {owner: ownerSet[item], facility: this.state.facilitiesAvailable }, this.props.siteUrl, _itemId);
      });
    }
    this.setState({
      isSavingDone: true,
    ...
    setTimeout(() => {
      ...
      this.setState({
        ...
        saveStart: false,
        failureMessage: "Some issue while creating request, Please contact admin.",
       });
      ...
      this.setState({
        isSavingFailure: true,
        saveStart: false,
        failureMessage: "This room is not available on the date and time that you've requested.",
       });
    }
 ...
 public Redirect = () => {
        ...
        {(formik) => {
          ...
          return (
            <form onSubmit={formik.handleSubmit}>
                      ...
                      >
                       ...
                       {formik.values.currentRecord >=0 && <Button
                        ...
                        <Button
                          ...
                          disabled={
                            !formik.values.facility || !formik.values.quantity
                          }
                ...
                <Grid container spacing={4}>
                    ...
                    <>
                      ...
                      <div className={styles.width}>
                        <div className={styles.label}>Principal User*</div>
                          <Dropdown items={princialList} name="principal" />
                        </div>
                      </Grid>
                  ...
                  <div className={styles.width}>
                    <div className={styles.label}>Participants</div>
                      ...
                      />
                     { formik.values.participant.length > 0 &&  showCSRDField && (
              ...
              <Snackbar open={isSavingFailure} onClose={() => this.setState({isSavingFailure : false})} >
                <Alert severity="error" onClose={() => this.setState({isSavingFailure : false})}>
                  {failureMessage}
                </Alert>
              </Snackbar>
            </form>
  ...
  const deparmentList: any[] = await sp.web.lists
    ...
    if(deparmentData.length === 0) {
      this.setState({
        failureMessage: 'User details is not present in department list, kindly contact admin.',
        isSavingFailure: true,
      });
      return;
    }
  ...
  public getFacility = async () => {
    ...
    Facility.forEach((item) => {
      facility[item.Facility] = { ...item, "FacilityOwner": item.FacilityOwner.results[0].EMail, };
    });
